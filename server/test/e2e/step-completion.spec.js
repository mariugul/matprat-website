...
test.describe('Step Completion', () => {
  ...
  test('should mark step as complete when clicked', async ({ page }) => {
    ...
    const hasCompleteClass = await firstStep.evaluate((el) => el.classList.contains('bg-success'));
  ...
  test('should toggle step completion on repeated clicks', async ({ page }) => {
    ...
    await firstStep.click();
    await page.waitForTimeout(100);
    let hasCompleteClass = await firstStep.evaluate((el) => el.classList.contains('bg-success'));
    expect(hasCompleteClass).toBe(true);

    // Click again - mark incomplete
    await firstStep.click();
    await page.waitForTimeout(100);
    hasCompleteClass = await firstStep.evaluate((el) => el.classList.contains('bg-success'));
  ...
  test('should allow multiple steps to be completed', async ({ page }) => {
    ...
    if (count >= 2) {
      ...
      const firstComplete = await steps.nth(0).evaluate((el) => el.classList.contains('bg-success'));
      const secondComplete = await steps.nth(1).evaluate((el) => el.classList.contains('bg-success'));
