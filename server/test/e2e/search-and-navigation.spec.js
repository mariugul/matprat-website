...
test.describe('Search and Navigation', () => {
  ...
  test('should show no results message when search has no matches', async ({ page }) => {
    ...
    if (visibleCount === 0) {
      ...
      // If search doesn't work, just verify we have the no-results element
      const noResultsText = page.locator('text=No recipes found');
      const noResultsCount = await noResultsText.count();
